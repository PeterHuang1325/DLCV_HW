...
def get_datasets(_dataset_dir, tfm):
    ...
    train_set = SVHN_Dataset(os.path.join(_dataset_dir,'data'), info=train_info, tfm=tfm)
    val_set = SVHN_Dataset(os.path.join(_dataset_dir,'data'), info=val_info, tfm=tfm)
...
def get_svhn(_dataset_dir, batch_size, train):
    ...
    if train:
        svhn_dataloader = train_loader
    else:
        svhn_dataloader = val_loader
