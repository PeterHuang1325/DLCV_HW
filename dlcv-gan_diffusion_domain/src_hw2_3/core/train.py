...
def train_src(model, params, src_data_loader, tgt_data_loader, tgt_data_loader_eval, device, logger):
    ...
    for epoch in range(params.num_epochs):
        ...
        if ((epoch + 1) % params.eval_step == 0):
            src_test_loss, src_acc, src_acc_domain, _ = test(model, src_data_loader, device, flag='source')
            tgt_test_loss, tgt_acc, tgt_acc_domain, _ = test(model, tgt_data_loader_eval, device, flag='target')
...
def train_tgt(model, params, src_data_loader, tgt_data_loader, tgt_data_loader_eval, device, logger):
    """Train dann."""
    ####################
    # 1. setup network #
    ####################
    ...
    for epoch in range(params.num_epochs):
        ...
        for step, ((_, _), (images_tgt, class_tgt)) in data_zip:
            ...
            # train on source domain
            tgt_class_output, tgt_domain_output = model(input_data=images_tgt, alpha=alpha)
...
def train_dann(model, params, src_data_loader, tgt_data_loader, tgt_data_loader_eval, device, logger):
    ...
    return model #, tgt_feat_list, src_feat_list 
