...
def load_data(args,json_path):
    ...
    if args.dataset_type == 'llff':
        ...
        poses, render_poses, hwf, i_split, filenames = load_blender_data(json_path, args.half_res, args.testskip)
        #images, poses, render_poses, hwf, i_split, filenames = load_blender_new(args.datadir, args.half_res, args.testskip)
        print('Loaded blender', render_poses.shape, hwf, json_path)
    ...
    H, W = int(H), int(W)
    hwf = [H, W, focal]
    HW = np.array([[800,800]]*len(filenames['test']))
    #HW = np.array([im.shape[:2] for im in images])
    
    irregular_shape = False#(images.dtype is np.dtype('object'))
