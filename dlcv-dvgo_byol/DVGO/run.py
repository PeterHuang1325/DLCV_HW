...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, file_names=None):
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            ...
            savename = os.path.join(savedir, file_names[i])
            imageio.imsave(savename, rgb8)
...
def load_everything(args, cfg):
    ...
    '''
    data_dict, filenames = load_data(cfg.data, args.json_path)
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    HW, Ks, near, far, i_train, i_test, poses, render_poses, images = [
...
if __name__=='__main__':
    ...
    args.config = './DVGO/configs/nerf/hotdog.py'
    ...
    # load images / poses / camera settings / data split
    data_dict, filenames = load_everything(args=args, cfg=cfg)
    ...
    if args.render_train:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
                ...
                eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg, file_names=filenames['train'],
                **render_viewpoints_kwargs)
    ...
    if args.render_test:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
                ...
                gt_imgs=None,
                savedir=testsavedir, dump_images=args.dump_images,
                eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg, file_names=filenames['test'],
                **render_viewpoints_kwargs)
        '''
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30, quality=8)
